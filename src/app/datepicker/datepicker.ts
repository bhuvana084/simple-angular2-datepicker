  ...
  @Input() value:string='';
	...
	setDateArray(month,year,date):any{
		...
		for (let i=1;i<=tempLastDate;i++){
            let currentDate = moment(new Date().toString(), 'MM/DD/YYYY').year(year).month(month).date(i);
            let pastDate = moment(this.minDate, 'MM/DD/YYYY');
            let futureDate = moment(this.maxDate, 'MM/DD/YYYY').add(1, 'd');
            let dbld = false;
	...
	setDate(sDate) {
        ...
        let selDate = moment().year(this.currYear).month(this.currMonth).date(sDate.date).format('MM/DD/YYYY');
